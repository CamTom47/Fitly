...
const WGER_BASE_URL : string = "https://wger.de/api/v2"
    ...
    class WgerApi {
        static async request( endpoint: string, data: object = {}, method: string = "get"){
            ...
            : {};
            try{
                const res : AxiosResponse = (await axios({url, method, data, params}));
                return res
            } catch(err){
        ...
        static async getAllExercises (endpoint : string = 'exercisebaseinfo/'): Promise<typeof res> {
            ...
            if(endpoint.includes(WGER_BASE_URL)) endpoint = endpoint.split(`${WGER_BASE_URL}/`)[1];
