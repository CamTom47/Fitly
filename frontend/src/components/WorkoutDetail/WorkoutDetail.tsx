...
const WorkoutDetail = (): React.JSX.Element => {
  ...
  const workout = workouts.find(
    (workout: Workout) => workout.id === Number(workoutId)
  );
  const circuits = useAppSelector(selectCircuits).filter(
    (circuit: CircuitType) => circuit.workoutId === workout.id
  );
  const categories = useAppSelector(selectCategories);
  const category = categories.find(
    (category: Category) => category.id === workout.category
  );
  ...
  return isLoading ? (
    ...
    <UpdateWorkoutForm
      key={uuid()}
      workout={workout}
      handleToggle={toggleShowWorkoutUpdateForm}
    />
  ) : (
    <div id="WorkoutDetailContainer">
      <div className="WorkoutDetailContainerInner">
        <div className="WorkoutDetailContainerHead">
          ...
          <div>
            <button
              className="WorkoutDetailButton"
              onClick={toggleShowWorkoutUpdateForm}
            >
        ...
        <div className="circuitSection">
          <div className="dividerSection1">
            ...
            id='addCircuitButton'
              className="faIcon"
              onClick={toggleShowNewCircuitForm}
            >+</button>
          </div>
          {showNewCircuitForm ? (
            <NewCircuitForm
              workout={workout}
              toggleShowNewCircuitForm={toggleShowNewCircuitForm}
            />
          ) : null}
          <div className="CircuitList">{circuitComponents}</div>
        </div>
      </div>
    </div>
  );
